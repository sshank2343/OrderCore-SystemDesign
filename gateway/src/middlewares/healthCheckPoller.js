...
// Periodically pings every downstream service's /health endpoint ("heartbeat") and keeps track of which ones are currently alive. This is the same mechanism real load balancers and service meshes use to detect a dead node BEFORE a real user request hits it and fails.→ [Heartbeats]
// Later (Project 3) this same concept gets extended into full [Failover] — this file only detects the problem, it doesn't yet reroute traffic away from a dead service.
...
// Exported so other parts of the gateway (or an admin dashboard later) can read current service health without re-polling.
...
async function pingSingleServiceHealth(serviceName, baseUrl, logger) {
    try {
        ...
        // Only log on state CHANGE, not every single successful heartbeat — otherwise the logs would be flooded every 5 seconds per service.   
        ...
        if(wasAlive !== false) {
            logger.warn(`${serviceName} failed heartbeat check: ${error.message}`);
        }
    }
}
