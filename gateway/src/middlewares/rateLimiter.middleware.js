...
async function consumeTokenForClient(clientKey) {
  ...
  // Lua script executed atomically inside Redis so concurrent requests from the
  // same client can't race each other and double-spend tokens.
  const luaScript = `
...
async function rateLimiterMiddleware(req, res, next) {
  ...
  try {
    ...
    // If Redis itself is down, fail open (allow the request) rather than
    // taking down the whole gateway because of a rate-limiter outage.
